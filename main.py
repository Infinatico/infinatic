...
#hashed = hashlib.sha512(TOKEN.encode('utf-8')).hexdigest()
#r = requests.get(url="https://nergon.unesty.net/licenses/license.php?key="+hashed, verify=False)
#if r.status_code != 200:
#    print("Error. Bot token is not registered for using ths bot. Please register your license in the official Discord Server")
#    print("Exiting...")
#    exit()
        ...
        if product["uniqid"] in stock_list:
            if stock_list[product["uniqid"]] != int(product["stock"]):
                ...
                if int(product["stock"]) == 1:
                    ...
                    await client.get_channel(restock_channel).send(embed=embed)
                elif stock_list[product["uniqid"]] < int(product["stock"]):
...
async def verify(ctx, order_id):
    ...
    with open("used.txt", "r") as f:
        for line in f:
            if line.strip() == order_id:
                embed = discord.Embed(title="Order already used",
                                      description="This order id was already used",
                                      color=discord.Color.red())
                await ctx.send(embed=embed)
                await ctx.message.delete()
                return
    ...
    if order_information["status"] != "COMPLETED":
...
async def checkorder(ctx, order_id):
    ...
    if order_information["status"] == "CUSTOMER_DISPUTE_ONGOING":
        status = "DISPUTE ONGOING"
    elif order_information["status"] == "WAITING_FOR_CONFIRMATIONS":
        status = "WATING FOR CONFIRMATIONS"
    else:
